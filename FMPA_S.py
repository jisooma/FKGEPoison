...
class Server(object):
    ...
    def poison_attack_random(self,  victim_client=0):
        ...
        for ent in attacked_entity_list:
            ...
            for i in range(256):
                poisoned_tri.append([int(ent),int(fake_r),int(fake_tail)])

        # 3.The poisoned triplets are saved in file
        self.poisoned_tri = poisoned_tri  # t_p
        dic = {}
        ...
        with open(
                self.args.poisoned_triples_path + self.args.dataset_name + '_' + self.args.client_model + '_' + str(
                        self.args.attack_entity_ratio) + '_'+str(self.args.num_client) +'_poisoned_triples_static_poisoned.json', 'w') as file1:
            json.dump(dic, file1)

        # 4、The server generate the training dataset D_p = {T_1 ∩ t_p}
        self.poisoned_triples = poisoned_tri + real_triples[victim_client]['train']
    ...
    def poison_aggregation(self, clients, ent_update_weights):
        ...
        # The poisoned entity embedding is appended. E_k+E^s_k
    ...
    def server_poison_static_update(self):
        ...
        for i in range(int(self.args.local_epoch)):
            # The shadow model of the server is trained using the training dataset (D_p = T_1 + t_p)
...
class FMPA_S(object):
    ...
    def server_static_attack(self):
        self.server.train_poison_model()
